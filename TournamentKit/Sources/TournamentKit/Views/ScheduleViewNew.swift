...
public struct ScheduleViewNew: View {
    @Environment(\.modelContext) private var modelContext
    @Environment(\.tournamentConfig) private var config
    @State private var viewModel: ScheduleViewModel

    public init(viewModel: ScheduleViewModel? = nil) {
                ...
                if !viewModel.hasFixtures && !viewModel.isLoading {
                    VStack(spacing: 12) {
                        Image(systemName: "calendar.badge.exclamationmark")
                            .font(.system(size: 60))
                            .foregroundColor(Color(config.accentColorName).opacity(0.5))
...
public struct FixtureCard: View {
    public let fixture: FixtureModel

    @Environment(\.tournamentConfig) private var config

    public init(fixture: FixtureModel) {
    ...
    private var homeScoreColor: Color {
        ...
        if fixture.homeTeamWinner { return Color(config.secondaryColorName) }
        if fixture.awayTeamWinner { return Color(config.accentColorName) }
        return .secondary
    }
    private var awayScoreColor: Color {
        if showsPenaltyScore { return .secondary }
        if fixture.awayTeamWinner { return Color(config.secondaryColorName) }
        if fixture.homeTeamWinner { return Color(config.accentColorName) }
        return .secondary
    }
    private var homePenaltyColor: Color {
        if fixture.penaltyHome > fixture.penaltyAway { return Color(config.secondaryColorName) }
        if fixture.penaltyHome < fixture.penaltyAway { return Color(config.accentColorName) }
        return .secondary
    }
    private var awayPenaltyColor: Color {
        if fixture.penaltyAway > fixture.penaltyHome { return Color(config.secondaryColorName) }
        if fixture.penaltyAway < fixture.penaltyHome { return Color(config.accentColorName) }
        return .secondary
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                HStack(spacing: 8) {
                    Text(fixture.formattedDate)
                        ...
                        .padding(.vertical, 4)
                        .background(Color(config.accentColorName).opacity(0.1))
                        .foregroundColor(Color(config.accentColorName))
                        .cornerRadius(4)
                        .overlay(
                            RoundedRectangle(cornerRadius: 4)
                                .stroke(Color(config.accentColorName), lineWidth: 1)
                        )
                ...
                VStack(spacing: 8) {
                    if let flagAsset = TeamFlagMapper.flagAssetName(for: fixture.homeTeamId) {
                        ...
                        Circle()
                            .fill(
                                LinearGradient(
                                    colors: [Color(config.accentColorName), Color(config.accentColorName).opacity(0.7)],
                ...
                VStack(spacing: 8) {
                    if let flagAsset = TeamFlagMapper.flagAssetName(for: fixture.awayTeamId) {
                        ...
                        Circle()
                            .fill(
                                LinearGradient(
                                    colors: [Color(config.secondaryColorName), Color(config.secondaryColorName).opacity(0.7)],
                                    startPoint: .topLeading,
                                    endPoint: .bottomTrailing
                                )
                            )
            ...
            HStack(spacing: 4) {
                Image(systemName: "mappin.circle.fill")
                    .font(.caption)
                    .foregroundColor(Color(config.accentColorName).opacity(0.7))
