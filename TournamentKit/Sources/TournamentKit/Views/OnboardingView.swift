...
public struct TeamSelectionPage: View {
        ...
        VStack(spacing: 24) {
            ...
            Button {
                ...
                Text("Continue")
                    ...
                    .padding()
                    .background(selectedTeams.isEmpty ? AnyShapeStyle(Color.gray.opacity(0.5)) : AnyShapeStyle(LinearGradient(
    ...
    var body: some View {
            ...
            Image(team.assetName)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .stroke(isSelected ? Color(config.accentColorName) : Color.clear, lineWidth: 3)
                )
                .shadow(color: isSelected ? Color(config.accentColorName).opacity(0.4) : .clear, radius: 6)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12)
                .fill(isSelected ? Color(config.accentColorName).opacity(0.15) : Color(UIColor.secondarySystemBackground))
        )
    }
}
...
public struct NotificationsIntroPage: View {
    @StateObject private var notificationService = AppNotificationService.shared
    @Environment(\.tournamentConfig) private var config
    @State private var isAuthorized: Bool = false
    @State private var isLoading: Bool = false
    public var onFinished: () -> Void
    public var onMaybeLater: () -> Void

    public init(onFinished: @escaping () -> Void, onMaybeLater: @escaping () -> Void) {
        self.onFinished = onFinished
    ...
    public var body: some View {
        VStack(spacing: 24) {
            ...
            Button {
                ...
                .frame(maxWidth: .infinity)
                .padding()
                .background(isAuthorized ? AnyShapeStyle(Color.gray.opacity(0.5)) : AnyShapeStyle(LinearGradient(
                    colors: [Color(config.accentColorName), Color(config.secondaryColorName)],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )))
