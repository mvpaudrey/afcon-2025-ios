...
public struct MatchCard: View {
    public let match: Game
    public var events: [Afcon_FixtureEvent] = []
    public var showsCompetition: Bool = true

    @Environment(\.tournamentConfig) private var config

    public init(match: Game, events: [Afcon_FixtureEvent] = [], showsCompetition: Bool = true) {
        self.match = match
        self.events = events
        self.showsCompetition = showsCompetition
    }
    ...
    private func scoreColor(for score: Int, opponentScore: Int) -> Color {
        if score > opponentScore {
            return Color(config.secondaryColorName)
        }
        if score < opponentScore {
            return Color(config.accentColorName)
        }
    ...
    public var body: some View {
        ...
        VStack(spacing: 12) {
            HStack {
                ...
                .padding(.vertical, 4)
                .background(
                    isBreak ? Color.orange : (isLive ? Color(config.accentColorName) : Color(.systemGray5))
                )
                .foregroundColor(
                    ...
                    if match.status == .upcoming && match.homeScore == 0 && match.awayScore == 0 {
                        ...
                        VStack(spacing: 4) {
                            ...
                            if let homePens = match.homePenaltyScore, let awayPens = match.awayPenaltyScore {
                                ...
                                .font(.subheadline)
                                .fontWeight(.semibold)
                                .foregroundColor(homePens > awayPens ? Color(config.secondaryColorName) : (awayPens > homePens ? Color(config.accentColorName) : .secondary))
                                .monospacedDigit()
                            }
                        }
                    }
    ...
    public struct MatchEventRow: View {
        public let event: Afcon_FixtureEvent
        public let isHomeEvent: Bool

        public init(event: Afcon_FixtureEvent, isHomeEvent: Bool) {
            self.event = event
            self.isHomeEvent = isHomeEvent
        }
