...
public final class ScheduleViewModel {
    ...
    public init(afconService: TournamentServiceWrapper = .shared, modelContext: ModelContext? = nil) {
    ...
    @MainActor
    public func loadFixturesFromDatabase() {
    ...
    @MainActor
    public func fetchAllFixtures() async {
        isLoading = true
        errorMessage = nil
    ...
    public func shouldRefreshFixtures() -> Bool {
    ...
    @MainActor
    public func fetchFixturesByDate(_ date: String) async {
        isLoading = true
        errorMessage = nil

        do {
            print("📡 Fetching fixtures for date: \(date)")
            let grpcFixtures = try await afconService.getFixtures(date: date)
    ...
    @MainActor
    public func performInitialSyncIfNeeded() async {
    ...
    public var upcomingFixtures: [FixtureModel] {
        fixtures.filter { $0.isUpcoming }
    }

    public var liveFixtures: [FixtureModel] {
        fixtures.filter { $0.isLive }
    }

    public var finishedFixtures: [FixtureModel] {
